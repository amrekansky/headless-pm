...
export async function promptMcpSetup() {
  ...
  console.log('\nWhich PM tools do you use? (MCP integration)')
  MCP_SERVERS.forEach((s, i) => console.log(`  [${i + 1}] ${s.label}`))
  console.log('  [A] All   [S] Skip')

  const answer = (await ask('\nEnter numbers separated by comma (or A/S): ')).trim().toUpperCase()
  rl.close()

  if (answer === 'S' || answer === '') return
  if (answer === 'A') return installMcpServers(null)

  const indices = answer.split(',').map(s => parseInt(s.trim(), 10) - 1).filter(i => i >= 0 && i < MCP_SERVERS.length)
  const names = indices.map(i => MCP_SERVERS[i].name)
  if (names.length) return installMcpServers(names)
}
