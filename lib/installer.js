...
function run(input) {
  ...
  if (remaining !== undefined) {
    const green = '\\x1b[32m', yellow = '\\x1b[33m', red = '\\x1b[31m', dimR = '\\x1b[2m', resetR = '\\x1b[0m';
    const color = remaining > 40 ? green : remaining > 20 ? yellow : red;
    ctxBar = \`\${dimR}ctx:\${resetR}\${color}\${remaining}%\${resetR}\`;
  }
  ...
  if (pmStatus && gsdStatus) {
    process.stdout.write(\`\${pmStatus}  \${ctxBar ? ctxBar + ' │ ' : '│ '}\${gsdStatus}\`);
  } else if (pmStatus) {
    process.stdout.write(ctxBar ? \`\${pmStatus}  \${ctxBar}\` : pmStatus);
  } else if (gsdStatus) {
    process.stdout.write(ctxBar ? \`\${ctxBar} │ \${gsdStatus}\` : gsdStatus);
  }
}
`;
