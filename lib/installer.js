...
try {
  ...
  pmStatus = focus ? \`[\${phase}] Sprint \${sprintN} · \${focus}\` : \`[\${phase}] Sprint \${sprintN}\`;
} catch {}

let gsdStatus = '';
try {
  gsdStatus = execFileSync('node', [${JSON.stringify(gsdPath)}], { encoding: 'utf8', timeout: 4000 }).trim();
} catch {}

if (pmStatus && gsdStatus) {
  process.stdout.write(\`\${pmStatus} │ \${gsdStatus}\`);
} else if (pmStatus) {
  process.stdout.write(pmStatus);
} else if (gsdStatus) {
