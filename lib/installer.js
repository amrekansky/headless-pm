...
async function injectGlobalContext(clis) {
  const targets = {
  ...
  for (const cli of clis) {
    ...
    let existing = ''
    try { existing = await readFile(filePath, 'utf8') } catch { existing = '' }
    if (existing.includes(CONTEXT_MARKER)) continue
    await mkdir(dirname(filePath), { recursive: true })
    await writeFile(filePath, existing + CONTEXT_BLOCK, 'utf8')
  }
}
...
export async function installSkills(licenseKey = null, clis = null) {
  ...
  if (!licenseKey && !TRIAL_MODE) {
    await injectGlobalContext(targetCLIs)
    console.log('\n  Unlock /pm orchestrator → headlesspm.com')
    console.log('  Already have a key? npx headless-pm setup --key=YOUR-KEY\n')
  }
}
