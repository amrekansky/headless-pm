...
function run(input) {
  ...
  const fiveH = ctx.rate_limits?.five_hour?.used_percentage != null ? \`5h:\${ctx.rate_limits.five_hour.used_percentage}%\` : '';
  ...
  if (pmStatus && gsdStatus) {
    ...
    const extras = [model, project, cost, fiveH].filter(Boolean).map(s => \`\${dim}\${s}\${reset}\`).join('  ');
