...
export async function installSkills(licenseKey = null) {
  ...
  // TRIAL_MODE: install all skills without license check — revert before production
...
export async function installMcpServers(serverNames = null) {
  ...
  for (const server of servers) {
    try {
      ...
      console.log(`  ~ ${server.label} (already registered — run 'claude mcp list' to verify)`)
    }
  }
  ...
  const oauthServers = servers.filter(s => s.transport === 'http')
  if (oauthServers.length) {
    console.log(`\nOAuth servers will prompt for browser login on first use in Claude Code.`)
  }
